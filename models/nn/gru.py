...
class DynamicGRU(nn.Module):
    ...
    def forward(self, packed_input, att_scores, hx=None):
        is_packed_input = isinstance(packed_input, PackedSequence)
        if not is_packed_input:
            raise NotImplementedError(
                "DynamicGRU only supports packed packed_input")
        ...
        packed_input, batch_sizes, sorted_indices, unsorted_indices = packed_input
        ...
        if hx is None:
            hx = torch.zeros(max_batch_size, self.hidden_size, dtype=packed_input.dtype)

        outputs = torch.zeros(packed_input.size(0), self.hidden_size, dtype=packed_input.dtype)

        begin = 0
        for batch in batch_sizes:
            new_hx = self.rnn(
                packed_input[begin: begin + batch],
                hx[0:batch],
